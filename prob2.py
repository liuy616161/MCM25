...
def optimize_problem2():
    ...
    if best_params:
        ...
        if fine_tuning_solutions:
            ...
            df_fine.to_excel(f"{results_dir}/fine_tuning_solutions.xlsx", index=False)
            print(f"已保存精细化搜索的解决方案到 {results_dir}/fine_tuning_solutions.xlsx，共 {len(fine_tuning_solutions)} 个方案")
...
def analyze_solutions(all_solutions):
    """分析所有解决方案的特性"""
    ...
    # 创建结果目录
    results_dir = "optimization_results/analysis"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    
    # 1. 统计各参数对有效时长的影响
    # 方向分析
    direction_analysis = df.groupby('direction')['effective_duration'].agg(['mean', 'max', 'count'])
    direction_analysis.to_excel(f"{results_dir}/direction_analysis.xlsx")
    
    # 速度分析
    speed_analysis = df.groupby('speed')['effective_duration'].agg(['mean', 'max', 'count'])
    speed_analysis.to_excel(f"{results_dir}/speed_analysis.xlsx")
    
    # 投放时间分析 - 将连续值分组
    df['release_time_bin'] = pd.cut(df['release_time'], bins=10)
    release_time_analysis = df.groupby('release_time_bin')['effective_duration'].agg(['mean', 'max', 'count'])
    release_time_analysis.to_excel(f"{results_dir}/release_time_analysis.xlsx")
    
    # 起爆延迟分析 - 将连续值分组
    df['detonation_delay_bin'] = pd.cut(df['detonation_delay'], bins=10)
    detonation_delay_analysis = df.groupby('detonation_delay_bin')['effective_duration'].agg(['mean', 'max', 'count'])
    detonation_delay_analysis.to_excel(f"{results_dir}/detonation_delay_analysis.xlsx")
    
    # 2. 创建可视化图表
    plt.figure(figsize=(12, 10))
    plt.subplot(2, 2, 1)
    direction_analysis['max'].plot(kind='bar')
    plt.title('不同方向的最大有效时长')
    plt.xlabel('方向 (°)')
    plt.ylabel('最大有效时长 (s)')
    plt.tight_layout()
    
    plt.subplot(2, 2, 2)
    speed_analysis['max'].plot(kind='bar')
    plt.title('不同速度的最大有效时长')
    plt.xlabel('速度 (m/s)')
    plt.ylabel('最大有效时长 (s)')
    plt.tight_layout()
    
    plt.subplot(2, 2, 3)
    release_time_analysis['max'].plot(kind='bar')
    plt.title('不同投放时间的最大有效时长')
    plt.xlabel('投放时间 (s)')
    plt.ylabel('最大有效时长 (s)')
    ...
    plt.subplot(2, 2, 4)
    detonation_delay_analysis['max'].plot(kind='bar')
    plt.title('不同起爆延迟的最大有效时长')
    plt.xlabel('起爆延迟 (s)')
    plt.ylabel('最大有效时长 (s)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    
    plt.savefig(f"{results_dir}/parameters_analysis.png", dpi=300)
    plt.close()
    
    # 3. 参数相关性分析
    corr = df[['direction', 'speed', 'release_time', 'detonation_delay', 'effective_duration']].corr()
    corr.to_excel(f"{results_dir}/parameters_correlation.xlsx")
    ...
    print(f"解决方案分析完成，结果保存在 {results_dir} 目录下")
...
if __name__ == "__main__":
    ...
    # 分析所有解决方案
    analyze_solutions(all_solutions)