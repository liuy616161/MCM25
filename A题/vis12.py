...
def optimize_problem2():
    ...
    best_direction = 0
    best_speed = 0
    best_effective_duration = 0
    best_params = None
    
    # 搜索不同的方向和速度组合
    for direction in range(0, 360, 5):  # 每5度搜索一次
        for speed in range(70, 141, 10):  # 70到140 m/s，每10 m/s搜索一次
            # 寻找最佳投放时间
            for release_time in np.linspace(1, 10, 10):
                for detonation_delay in np.linspace(2, 6, 9):
                    detonation_time = release_time + detonation_delay
                    
                    # 计算投放点
                    drone_pos = drone_trajectory(DRONE_FY1_INIT, direction, speed, release_time)
                    
                    # 计算起爆点
                    bomb_pos = smoke_trajectory(drone_pos, release_time, detonation_time)
                    ...
                    # 简化的计算，以0.1秒为步长
                    for t in np.arange(0, 80, 0.1):
                        missile_pos = missile_trajectory(MISSILE_M1_INIT, FAKE_TARGET, MISSILE_SPEED, t)
                        
                        if t >= detonation_time:
                            cloud_pos = smoke_cloud_trajectory(bomb_pos, detonation_time, t)
                            
                            # 计算导弹与云团的距离
                            if cloud_pos is not None:
                                distance = np.linalg.norm(missile_pos - cloud_pos)
                                effective = (distance <= SMOKE_EFFECTIVE_RADIUS and 
                                            t - detonation_time <= SMOKE_EFFECTIVE_TIME)
                                
                                if effective and not continuous:
                                    continuous = True
                                    start_time = t
                                elif not effective and continuous:
                                    continuous = False
                                    effective_duration = max(effective_duration, t - start_time)
                    
                    if effective_duration > best_effective_duration:
                        best_effective_duration = effective_duration
    ...
    print(f"问题2优化结果:")
    print(f"  最佳飞行方向: {best_direction}°")
    print(f"  最佳飞行速度: {best_speed} m/s")
    print(f"  最佳投放时间: {best_params['release_time']:.2f} s")
    print(f"  最佳起爆时间: {best_params['detonation_time']:.2f} s")
    print(f"  最佳投放点: ({best_params['drone_pos'][0]:.2f}, {best_params['drone_pos'][1]:.2f}, {best_params['drone_pos'][2]:.2f})")
    print(f"  最佳起爆点: ({best_params['bomb_pos'][0]:.2f}, {best_params['bomb_pos'][1]:.2f}, {best_params['bomb_pos'][2]:.2f})")
    print(f"  有效遮蔽时长: {best_effective_duration:.2f} s")
    
    # 可视化最佳结果
    visualize_solution(best_params, "问题2: 优化后的烟幕干扰弹投放策略")

def visualize_solution(params, title):
    """可视化给定参数的解决方案"""
    ...
    # 模拟导弹和烟幕的轨迹
    simulation_time = np.linspace(0, 80, 1000)
    missile_positions = []
    drone_positions = []
    smoke_bomb_positions = []
    smoke_cloud_positions = []
    ...
    for t in simulation_time:
        ...
        # 无人机位置
        drone_pos = drone_trajectory(DRONE_FY1_INIT, direction, speed, t)
        drone_positions.append(drone_pos)
        
        # 烟幕弹位置
        if t >= release_time and t < detonation_time:
            bomb_pos = smoke_trajectory(params['drone_pos'], release_time, t)
            smoke_bomb_positions.append(bomb_pos)
        
        # 烟幕云团位置
        if t >= detonation_time:
            cloud_pos = smoke_cloud_trajectory(params['bomb_pos'], detonation_time, t)
            smoke_cloud_positions.append((t, cloud_pos))
    ...
    # 绘制烟幕弹投放点
    ax.scatter(params['drone_pos'][0], params['drone_pos'][1], params['drone_pos'][2], 
               color='orange', s=80, label='烟幕弹投放点')
    
    # 绘制烟幕弹起爆点
    ax.scatter(params['bomb_pos'][0], params['bomb_pos'][1], params['bomb_pos'][2], 
               color='purple', s=80, label='烟幕弹起爆点')
    ...
    plt.tight_layout()
    plt.savefig('problem2_visualization.png', dpi=300)
    plt.show()
