...
def fine_tune_solution(base_solution, solution_id, results_dir):
    ...
    # 定义搜索范围
    direction_range = np.linspace(direction - 2, direction + 2, 5)
    speed_range = np.linspace(max(70, speed - 2), min(140, speed + 2), 5)
    release_time_range = np.linspace(max(0.1, release_time - 0.5), release_time + 0.5, 5)
    detonation_delay_range = np.linspace(max(0.1, detonation_delay - 0.5), detonation_delay + 0.5, 5)
...
def main():
    # 创建结果目录
    results_dir = "FY3_finetune"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    
    # 读取前10个最佳解决方案
    try:
        df_top10 = pd.read_excel("./top10_solutions.xlsx")
