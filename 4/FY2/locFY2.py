...
DRONE_FY1_INIT = np.array([12000, 1400, 1400])
...
def optimize_problem2():
    ...
    # 创建结果目录
    results_dir = "FY2_optimization_results"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    
    directions = np.linspace(240, 300, 21) #41

    # 搜索空间
    for direction in directions: 
        start_direction_time = time.time()
        for speed in np.linspace(100, 140, 11):  #26# 70到140 m/s，每2.5 m/s搜索一次
            for release_time in np.linspace(3, 8, 11):  #11 
                for detonation_delay in np.linspace(3, 8, 11): #11  
