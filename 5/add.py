...
def evaluate_defense_balance(missile_times):
    """评估防御的平衡性，考虑三个导弹的遮蔽时间分布"""
    if not missile_times or min(missile_times) <= 0:
        return 0
    
    # 计算标准差与平均值的比值（变异系数）
    mean_time = sum(missile_times) / len(missile_times)
    variance = sum((t - mean_time) ** 2 for t in missile_times) / len(missile_times)
    std_dev = variance ** 0.5
    cv = std_dev / mean_time if mean_time > 0 else float('inf')
    
    # 计算平衡系数 (0到1之间，越接近1表示越平衡)
    balance_factor = 1 / (1 + cv)
    
    return balance_factor

def evaluate_combined_effect(bombs, missile_count=3):
    """评估烟幕弹组合的整体效果，考虑总时长和平衡性"""
    # 提取所有遮蔽区间
    all_intervals = {i: [] for i in range(missile_count)}
    for bomb in bombs:
        missile_idx = bomb['target_missile']
        all_intervals[missile_idx].extend(bomb['intervals'])
    
    # 计算每个导弹的总有效遮蔽时长
    missile_times = []
    for missile_idx in range(missile_count):
        total_time = calculate_total_effective_time(all_intervals[missile_idx])
        missile_times.append(total_time)
    
    # 基础效果 = 总时长
    total_effect = sum(missile_times)
    
    # 计算平衡系数
    balance_factor = evaluate_defense_balance(missile_times)
    
    # 综合评分 = 总时长 × (1 + 平衡性权重 × 平衡系数)
    # 平衡性权重设定为0.5，可根据需要调整
    balance_weight = 0.5
    combined_score = total_effect * (1 + balance_weight * balance_factor)
    
    # 额外奖励：如果所有导弹都有非零遮蔽，给予额外奖励
    if all(t > 0 for t in missile_times):
        combined_score *= 1.2
    
    return combined_score, missile_times

def optimize_defense_strategy(num_combinations_per_drone=3, top_params_per_missile=2):
    ...
    for drone_idx in range(5):
        ...
        for missile_idx in range(3):
            solutions = get_top_solutions_with_intervals(
                drone_idx, missile_idx, 
                num_solutions=num_combinations_per_drone
            )
            if solutions:
    ...
    for drone_idx in range(1, 5):
        ...
        print(f"为无人机FY{drone_idx+1}生成组合策略...")
        ...
        print(f"  - 发现{len(direction_speed_options)}个可能的方向-速度组合")
        
        # 2. 对每个方向和速度组合，综合考虑针对不同导弹的参数组合
        for direction, speed in direction_speed_options:
            # 收集针对每个导弹的多个候选解决方案
            missile_solutions = {0: [], 1: [], 2: []}
            ...
            for missile_idx in range(3):
                if missile_idx in drone_missile_solutions[drone_idx]:
                    ...
                    top_solutions = sorted(
                        matching_solutions, 
                        key=lambda x: x['effective_duration'], 
                        reverse=True
                    )[:top_params_per_missile]  # 保留多个候选方案
                    
                    missile_solutions[missile_idx] = top_solutions
            
            # 3. 创建所有可能的导弹目标组合（考虑最多3个导弹）
            missile_combinations = []
            for r in range(1, min(4, sum(1 for m in missile_solutions.values() if m) + 1)):
                # 获取有候选解决方案的导弹索引
                available_missiles = [m for m in range(3) if missile_solutions[m]]
                for missile_targets in combinations(available_missiles, r):
                    missile_combinations.append(missile_targets)
            
            # 4. 对每个导弹组合，考虑参数的多种可能性
            for missile_targets in missile_combinations:
                # 创建参数组合
                param_options = []
                for missile_idx in missile_targets:
                    solutions = missile_solutions[missile_idx]
                    if solutions:  # 应该总是为真
                        param_options.append(solutions)
                
                # 生成所有可能的参数组合
                for param_combo in product(*param_options):
                    # 创建烟幕弹列表
                    bombs = []
                    for i, solution in enumerate(param_combo):
                        missile_idx = missile_targets[i]
                        bombs.append({
                            'release_time': solution['release_time'],
                            'detonation_delay': solution['detonation_delay'],
                    ...
                    # 5. 评估组合效果
                    combined_score, missile_times = evaluate_combined_effect(bombs)
        ...
        # 对当前无人机的所有组合按综合评分排序，选择前100个最佳组合
        # 这样可以减少后续笛卡尔积的规模，提高计算效率
        if len(drone_combinations[drone_idx]) > 100:
            drone_combinations[drone_idx] = sorted(
                drone_combinations[drone_idx],
                key=lambda x: x['combined_score'],
                reverse=True
            )[:100]
        
        print(f"  - 已为无人机FY{drone_idx+1}生成{len(drone_combinations[drone_idx])}个策略组合")
    ...
    # 定义全局评估函数
    def evaluate_global_combination(combo):
        # 每个导弹的所有烟幕弹遮蔽区间
        ...
        # 综合评分 = 总时长 × (1 + 平衡性权重 × 平衡系数)
        balance_weight = 0.5
        combined_score = total_time * (1 + balance_weight * balance_factor)
    ...
    all_results = Parallel(n_jobs=num_cores)(
        delayed(evaluate_global_combination)(combo) 
    ...
    # 选择最佳策略（优先选择对所有导弹都有遮蔽的方案）
    best_result = None
    for result in sorted_results:
        if all(t > 0 for t in result['times_by_missile']):
            best_result = result
            break
    
    # 如果没有满足所有导弹都有遮蔽的方案，则选择评分最高的方案
    if best_result is None:
        best_result = sorted_results[0]
    
    # 输出最佳策略
    print("\n最佳防御策略:")
    print(f"综合评分: {best_result['combined_score']:.2f}")
    print(f"总有效遮蔽时长: {best_result['total_time']:.2f}秒")
    print(f"平衡系数: {best_result['balance_factor']:.4f}")
    ...
    best_df = pd.DataFrame([{
        ...
        'balance_factor': best_result['balance_factor'],
...
if __name__ == "__main__":
    ...
    optimize_defense_strategy(num_combinations_per_drone=3, top_params_per_missile=2)
    end_time = time.time()
