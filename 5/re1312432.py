...
# Numba 优化版本的阴影锥判断函数
@njit(parallel=True)
def is_target_in_shadow_cone_numba(missile_pos, cloud_pos, target_center, target_radius, target_height):
    """Numba加速版本的阴影锥判断函数"""
    ...
    distance_missile_cloud = np.sqrt(direction[0]**2 + direction[1]**2 + direction[2]**2)
    ...
    # 检查目标中心点
    target_center_vector = target_center - missile_pos
    distance_center = np.sqrt(target_center_vector[0]**2 + target_center_vector[1]**2 + target_center_vector[2]**2)
    
    if distance_center > 0:
        cos_center_angle = (target_center_vector[0]*unit_direction[0] + 
                            target_center_vector[1]*unit_direction[1] + 
                            target_center_vector[2]*unit_direction[2]) / distance_center
    ...
    # 生成采样点
    num_points = 12
    all_points = np.zeros((num_points*2, 3))
    ...
    for i in range(num_points):
        ...
        all_points[i, 0] = x
        all_points[i, 1] = y
        all_points[i, 2] = target_center[2]
        
        # 顶面点
        all_points[i+num_points, 0] = x
        all_points[i+num_points, 1] = y
        all_points[i+num_points, 2] = target_center[2] + target_height
    
    # 检查所有点
    for i in prange(len(all_points)):
        point = all_points[i]
        vector = point - missile_pos
        distance = np.sqrt(vector[0]**2 + vector[1]**2 + vector[2]**2)
        
        if distance < 1e-10:
            continue
            
        cos_angle = (vector[0]*unit_direction[0] + 
                     vector[1]*unit_direction[1] + 
                     vector[2]*unit_direction[2]) / distance
        
        if cos_angle > 0:
            return False
        
        if abs(cos_angle) < abs(cos_theta):
            return False
    
    return True

# 普通版本的阴影锥判断函数 (用于不支持Numba的场景)
...
def calculate_shielding_effectiveness(missile_idx, missile_pos, cloud_pos, cloud_start_time, current_time):
    ...
    # 检查真目标是否被烟幕遮蔽 - 使用Numba加速版本
    try:
        target_shielded = is_target_in_shadow_cone_numba(
...
# 顶层函数 - 用于joblib并行处理
def evaluate_missile_defense(missile_idx, defense_plan, use_precomputed=False):
    """评估单个导弹的防御效果 - 顶级函数可以被序列化"""
    # 创建一个简单的计算环境
    simulator = DefenseSimulator(use_precomputed=use_precomputed)
    intervals = simulator.calculate_missile_defense_effect(
        missile_idx, defense_plan
    )
    effective_time = calculate_total_effective_time(intervals)
    return missile_idx, effective_time, intervals

# 顶层函数 - 用于并行评估个体
def evaluate_individual_parallel(individual, use_precomputed=False):
    """并行评估单个个体 - 顶级函数可以被序列化"""
    # 解析防御计划
    defense_plans = {0: [], 1: [], 2: []}
    ...
    # 对每个导弹并行评估
    tasks = [(missile_idx, defense_plans[missile_idx], False) 
             for missile_idx in range(3)]
    
    results = []
    for task in tasks:
        results.append(evaluate_missile_defense(*task))
    
    # 整合结果
    times_by_missile = [0, 0, 0]
    intervals_by_missile = {}
    
    for missile_idx, effective_time, intervals in results:
        times_by_missile[missile_idx] = effective_time
        intervals_by_missile[missile_idx] = intervals
    
    # 计算总有效遮蔽时长
    total_effective_time = sum(times_by_missile)
    
    # 计算平衡性指标
    balance_reward = 0
    min_time = min(times_by_missile)
...
class DefenseSimulator:
    ...
    def get_missile_position(self, missile_idx, t):
        """获取导弹在时间t的位置"""
        if self.use_precomputed and self.missile_trajectories:
            # 使用预计算轨迹 - 找到最接近的时间点
    ...
    def calculate_missile_defense_effect(self, missile_idx, defense_plan, time_step=0.01):
        ...
        # 导弹到达假目标的时间
        missile_total_time = self.missile_total_times[missile_idx] if hasattr(self, 'missile_total_times') else \
                             missile_time_to_target(MISSILE_POSITIONS[missile_idx], FAKE_TARGET, MISSILE_SPEED)
    ...
    def calculate_defense_effectiveness(self, defense_plans):
        """计算整体防御效果"""
        # 使用joblib并行计算每个导弹的防御效果
        import os
        
        # 获取CPU核心数
        n_jobs = min(os.cpu_count(), 3)  # 最多使用3个核心，因为只有3个导弹
        
        # 准备参数
        tasks = [(missile_idx, defense_plans[missile_idx], False) for missile_idx in range(3)]
        
        # 并行执行
        results = Parallel(n_jobs=n_jobs, backend="threading")(
            delayed(evaluate_missile_defense)(*task) for task in tasks
        )
        ...
        return {
            'total_time': sum(times_by_missile),
            'times_by_missile': times_by_missile,
...
class GeneticOptimizer:
    ...
    def __init__(self, simulator, config=None):
        ...
        default_config = {
            ...
            'seed_solutions': True,
            'num_threads': 4,  # 线程池大小
            'batch_size': 5,   # 并行评估批次大小
            'num_processes': 0  # 进程数，0表示自动选择
        }
        ...
        if self.config['num_processes'] <= 0:
            self.config['num_processes'] = min(20, multiprocessing.cpu_count())
    ...
    # 修改 evaluate_population_parallel 方法中的并行处理部分
    def evaluate_population_parallel(self, population):
        """并行评估整个种群 - 使用线程池而非进程池"""
        batch_size = self.config['batch_size']
        n_processes = self.config['num_processes']
        ...
        for batch_idx, batch in enumerate(batches):
            print(f"评估批次 {batch_idx+1}/{len(batches)} ({len(batch)}个体)...")
            
            # 修改为使用线程池而非进程池
            batch_results = Parallel(n_jobs=min(len(batch), n_processes), 
                                    backend="threading", verbose=0)(
                delayed(evaluate_individual_parallel)(ind, False) for ind in batch
            )
    ...
    def run(self, run_id="default"):
        ...
        self.evaluate_population_parallel(population)
        ...
        for generation in range(self.config['generations']):
            ...
            # 评估新一代中需要重新评估的个体 - 使用joblib并行
            invalid_ind = [ind for ind in offspring if not ind.fitness.valid]
            if invalid_ind:
                print(f"需要评估 {len(invalid_ind)} 个新个体...")
                self.evaluate_population_parallel(invalid_ind)
...
def main():
    ...
    final_config['generations'] = 50
    final_config['population_size'] = 50
    final_config['runs'] = 1
    final_config['num_processes'] = 20  # 利用所有核心
