...
def parallel_defense_evaluation(defense_plans, simulator):
    """使用混合并行策略评估防御计划"""
    from joblib import Parallel, delayed
    
    # 第一级并行：每个导弹分配一个进程
    def evaluate_missile(missile_idx):
    ...
    results = Parallel(n_jobs=-1, backend="multiprocessing")(
        delayed(evaluate_missile)(missile_idx) for missile_idx in range(3)
    )
...
class GeneticOptimizer:
    ...
    def evaluate_individual(self, individual):
        ...
        # 使用混合并行策略
        results = parallel_defense_evaluation(defense_plans, self.simulator)
    ...
    def run(self, run_id="default"):
        ...
        with ThreadPoolExecutor(max_workers=self.config['num_threads']) as executor:
            futures = [executor.submit(self.toolbox.evaluate, ind) for ind in population]
            for i, future in enumerate(tqdm(futures, desc="评估初始种群")):
                population[i].fitness.values = future.result()
        ...
        for generation in range(self.config['generations']):
            ...
            # 评估新一代中需要重新评估的个体 - 使用线程池
            invalid_ind = [ind for ind in offspring if not ind.fitness.valid]
            
            with ThreadPoolExecutor(max_workers=self.config['num_threads']) as executor:
                futures = [executor.submit(self.toolbox.evaluate, ind) for ind in invalid_ind]
                for i, future in enumerate(tqdm(futures, desc=f"评估第{generation+1}代")):
                    invalid_ind[i].fitness.values = future.result()
...
def main():
    ...
    final_config['runs'] = 1
    final_config['num_threads'] = 24  # 增加线程数
